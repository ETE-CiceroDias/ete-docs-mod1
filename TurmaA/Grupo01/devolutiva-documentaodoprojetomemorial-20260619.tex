...
% ── METADADOS ────────────────────────────────────────────────
\title{Feedback Avaliativo de Projeto: Projeto Integrador I --- Documentação do Projeto (Memorial)}
...
\midrule
Estrutura e Organização do Documento & \statuswarn{Não entregue} & \textbf{0,00} & 1,50 \\
Linguagem e Qualidade Acadêmica & \statuswarn{Não entregue} & \textbf{0,00} & 1,50 \\
Formatação e Padronização & \statuswarn{Não entregue} & \textbf{0,00} & 0,50 \\
Embasamento e Referências & \statuswarn{Não entregue} & \textbf{0,00} & 0,50 \\
\midrule
...
\begin{itemize}[leftmargin=14pt,itemsep=1pt,topsep=3pt,parsep=0pt]
  \item[$\square$] {\small\textcolor{cmuted}{Todas as seções esperadas estão presentes (capa, introdução, fases, conclusão, referências)}}
  \item[$\square$] {\small\textcolor{cmuted}{As seções seguem ordem lógica e o documento flui entre as fases}}
  \item[$\square$] {\small\textcolor{cmuted}{Há capa e sumário; a hierarquia de títulos é clara}}
  \item[$\square$] {\small\textcolor{cmuted}{Sem placeholders, lorem ipsum ou conteúdo de template não preenchido}}
  \item[$\square$] {\small\textcolor{cmuted}{O conteúdo de cada fase está na seção correta (sem Imersão/Definição trocadas)}}
\end{itemize}
...
\begin{itemize}[leftmargin=14pt,itemsep=1pt,topsep=3pt,parsep=0pt]
  \item[$\square$] {\small\textcolor{cmuted}{Registro acadêmico adequado — sem informalidade ou gírias}}
  \item[$\square$] {\small\textcolor{cmuted}{Texto claro, coeso e coerente do início ao fim}}
  \item[$\square$] {\small\textcolor{cmuted}{Ortografia e gramática revisadas}}
  \item[$\square$] {\small\textcolor{cmuted}{As ideias se conectam: o problema apresentado se sustenta até a solução}}
\end{itemize}
...
\begin{itemize}[leftmargin=14pt,itemsep=1pt,topsep=3pt,parsep=0pt]
  \item[$\square$] {\small\textcolor{cmuted}{Formatação consistente (fontes, espaçamento, margens)}}
  \item[$\square$] {\small\textcolor{cmuted}{Figuras, tabelas e quadros numerados e legendados}}
  \item[$\square$] {\small\textcolor{cmuted}{Identidade visual coerente ao longo do documento}}
...
\begin{itemize}[leftmargin=14pt,itemsep=1pt,topsep=3pt,parsep=0pt]
  \item[$\square$] {\small\textcolor{cmuted}{Afirmações relevantes estão referenciadas}}
  \item[$\square$] {\small\textcolor{cmuted}{Bibliografia presente e formatada}}
  \item[$\square$] {\small\textcolor{cmuted}{Fontes confiáveis e atuais}}
  \item[$\square$] {\small\textcolor{cmuted}{Citações no corpo do texto correspondem às referências}}
\end{itemize}
...
\toprule
\textbf{Integrante} & \textbf{\small PI Documentação do Projeto (Memorial)} & \textbf{Total} \\
\midrule
